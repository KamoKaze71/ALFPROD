...
function c1g_fillColPos(colHitTest, th)
{
	...
	{
		var colheader = tr.children[i];
		colHitTest[i] = new c1g_hitTestInfo(colheader);
	}	
}
...
function c1g_init(gridid, scrollX, scrollY)
{
	...
	if (props.keepsize)
	{
		...
		props.scrollcontainer.obj.attachEvent("onresize", c1g_scrollDivResize)
	}
	...
	var scroller = c1g_getElementById(gridid + "_scrolldiv");
	if (scroller)
	{
...
{
	if( props.objDragItem == null )
	{
		props.objDragItem = document.createElement("DIV");
		document.body.appendChild(props.objDragItem);
	}
	props.objDragItem.style.display		= "none";
	props.objDragItem.style.zIndex		= 1000;
	...
	props.objDragItem.style.fontFamily	= src.currentStyle.fontFamily;
	props.objDragItem.style.fontStyle	= src.currentStyle.fontStyle;
	props.objDragItem.style.fontWeight	= src.currentStyle.fontWeight;
	props.objDragItem.style.fontSize	= src.currentStyle.fontSize;
	props.objDragItem.style.textAlign	= src.currentStyle.textAlign;
...
function c1g_scrollDivResize(e)
{
	...
	var gn = c1g_getattr(e.srcElement, "id");
	var idx = rsearch(gn, "_");
...
function c1g_colHeaderMouseDown(e)
{
	...
	if (props.objResizeItem != null)
	{
	  if (!props.allowColSizing) return;
		...
		var o = document.getElementById(c1g_curGrid + "_GroupContainer");
		if (o == null)
			o = document.getElementById(c1g_curGrid);
		
		props.offsetWidth =	o.offsetWidth;
	}
	else
	{
...
function c1g_hitTestCols(gridid, x, y)
{
	...
	for (var i = 0; i < count; i++)
	{
		...
		var pos = (props.isLTR) ? props.colHitTest[i].left : props.colHitTest[i].right;
	...
	if (props.grpSrcIdx != -1)
	{
		...
		if (count >= 0)
		{
			posX = (props.isLTR) ? props.colHitTest[count].right : props.colHitTest[count].left;
		...
		else
		{
			...
			posX = (props.isLTR) ? c1g_findPosX(dataGrid) : c1g_findPosX(dataGrid) + dataGrid.offsetWidth;
			posY = c1g_findPosY(dataGrid);
			offsetHeight = 20;		
		}

		var out = (props.isLTR) ? (x > posX) : (x < posX);
		if (out && (y > posY) && (y < (posY + offsetHeight)))	
		{
		...
		var out2 = (props.isLTR) ? (x < posX + props.dataGrid.offsetWidth) : (x > posX - props.dataGrid.offsetWidth);
		if (count < 0 && out && out2 && (y > posY) && (y < posY + props.dataGrid.offsetHeight))
		{
			props.dstIdx = 0;
			return true;
		}
...
function c1g_hitTestGroup(gridid, x, y)
{
	...
	{  
		if (c1g_inCol(x, y, props.groupHitTest[i]))
		{
			...
			if (props.grpSrcIdx != -1 && props.grpSrcIdx < i)
				props.grpDstIdx = i - 1;
			else
				props.grpDstIdx = i;	

			var pos = (props.isLTR) ? pos = props.groupHitTest[i].left : pos = props.groupHitTest[i].right;
			if (props.grpCount == 0)
				pos -= (up.obj.offsetWidth / 2);

			c1g_showColInsPt(gridid, pos, props.groupHitTest[i].top, props.groupHitTest[0].srcElem.offsetHeight);
			return true;
		}
	}
	...
	if (props.grpCount > 0)
	{
		...
		if (props.isLTR)
		{
			edge = props.groupHitTest[count - 1].right;
			out = (x > edge);
		}
		else
		{	
			edge = props.groupHitTest[count - 1].left;
			out = (x < edge);
		}
		
		if (out && (y > props.groupInfo.top) && (y < (props.groupInfo.top + props.groupInfo.srcElem.offsetHeight)))
		{
			props.grpDstIdx = count;
			c1g_showColInsPt(gridid, edge, props.groupHitTest[count - 1].top, props.groupHitTest[count - 1].srcElem.offsetHeight);
			return true;
		}
	}
...
function c1g_chooseCursor(e)
{
	...
	if (!c1g_resizeMode && state)
	{
		...
		if (srcElem.tagName == "TH")
		{
			...
			if (posx >= edge - 4 && posx <= edge + 4  && c1g_canSizing(srcElem))
			{
			...
			if (state.allowColMoving && c1g_canMove(srcElem))   
...
function c1g_onMouseMove(e)
{
	...
	if (c1g_resizeMode)
	{
		...
		if (newwidth > 0)
		{
			...
			var gc = document.getElementById(c1g_curGrid + "_GroupContainer");
			if (gc != null)
				gc.style.width = props.offsetWidth + newwidth - props.resizeWidth;
				
			document.getElementById(c1g_curGrid).style.width = props.offsetWidth + newwidth - props.resizeWidth;	
			props.objResizeItem.style.width = newwidth;
			col.width = newwidth;
	...
	else if (c1g_dragMode)
	{
		...
		props.objDragItem.style.pixelTop  = e.clientY - midHObj;
		props.objDragItem.style.pixelLeft = e.clientX - midWObj;
		
		if	(props.objDragItem.style.display == "none") 
			props.objDragItem.style.display = "";

		if (!c1g_hitTestCols(c1g_curGrid, e.clientX + scrollx, e.clientY + scrolly) && c1g_canGroup(props.srcIdx))
		{
			// check group move
...
function c1g_canGroup(idx)
{
	if (idx > -1)
	{
		var colGroup = document.getElementById(c1g_curGrid).childNodes[0];
...
function c1g_canMove(elem)
{
	var colGroup = elem.parentNode.parentNode.previousSibling;
	var col = colGroup.childNodes[c1g_getHeadIdx(elem)];
	return (c1g_getattr(col, "AllowMove") == null) ? true : false;
}



function c1g_canSizing(elem)
{
	var colGroup = elem.parentNode.parentNode.previousSibling;
	var col = colGroup.childNodes[c1g_getHeadIdx(elem)];
	return (c1g_getattr(col, "AllowSizing") == null) ? true : false;
}



function c1g_reset(props)
{
	if (props != null)
	{
		props.dstIdx = -1;
...
function c1g_onMouseUp(e)
{
	...
	if (c1g_dragMode)
	{
		...
		props.objDragItem.style.display = "none";
		props.objDragItem.style.pixelTop = 0;
		props.objDragItem.style.pixelLeft = 0;

		if (props.srcIdx != -1)
		{
			if (props.dstIdx != -1)
				__doPostBack(c1g_getUniqueID(c1g_curGrid), "ColMove:"+props.srcIdx+":"+props.dstIdx);
			else if (props.grpDstIdx != -1 && c1g_canGroup(props.srcIdx))
			{
	...
	else if (c1g_resizeMode)
	{
		...
		var props = c1g_Props[c1g_curGrid];	
		//c1g_initHeadPos(props);
		c1g_reinit(c1g_curGrid);	

		var newWidth = props.resizeWidth - (props.posX - e.clientX);
	
		if (newWidth < 1)
			newWidth = 1;
		props.resizedColumns[props.srcIdx] = newWidth;
		c1g_reset(props);
		
		var val = "";
		for (var i in props.resizedColumns)
			val += i + ':' + props.resizedColumns[i] + ':';
		
		var o = c1g_getElementById(c1g_curGrid + "_GroupContainer");
		if (o == null)
			o = c1g_getElementById(c1g_curGrid);

		val = val + o.style.width;
		
		c1g_getElementById("__" + c1g_curGrid + "_ResizedColumns").value = val;
 	}
...
function c1g_onNoSplitScroll(gridId)
{
  var o = c1g_getElementById("__" + gridId + "_Scroll");
	if (o)
	{
	  var scroller = c1g_getElementById(gridId + "_scrolldiv");
		if (scroller)
