...
function c1g_showDrag(hdr, gridid, x, y, ishdr)
{
	try
	{
	  dragItem.obj = document.getElementById("~~dragElem");
	  dragItem.obj.innerHTML = hdr.firstChild.firstChild.innerHTML;
	  ...
	  dragItem.obj.style.top = y - (parseInt(dragItem.obj.style.height) / 2);;
          ...
          dragItem.obj.style.color = hdr.firstChild.color;
...
function c1g_mouseInGrid(e, gridid)
{
	...
	if (grid)
	{
	   ...
	   if ((e.pageX > pX) && (e.pageX < pX + grid.offsetWidth) &&
              (e.pageY > pY) && (e.pageY < pY + grid.offsetHeight))
           return true;
	}
...
function c1g_mouseInPanel(e, gridid)
{
	...
	if (panel)
	{
	  ...
	  if ((e.pageX > pX) && (e.pageX < pX + panel.offsetWidth) &&
              (e.pageY > pY) && (e.pageY < pY + panel.offsetHeight))
	  {
	     for (var i = 0; i < c1g_groupCount(gridid); i++)
	     {
	        ...
	        pY = c1g_findPosY(item);
                ...
                if ((e.pageX > pX) && (e.pageX < pX + item.offsetWidth) &&
                    (e.pageY > pY) && (e.pageY < pY + item.offsetHeight))
                return item;
	     }
	     return null;
	  }
...
function c1g_mouseInHeader(e, gridid)
{
	...
	if ((e.pageX > pX) && (e.pageX < pX + hpanel.offsetWidth) &&
           (e.pageY > pY) && (e.pageY < pY + hpanel.offsetHeight))
	{
	  ...
	  for (var i = 0; i < hpanel.cells.length; i++)
	  {
	     ...
	     if ((e.pageX > pX) && (e.pageX < pX + h.offsetWidth) &&
                (e.pageY > pY) && (e.pageY < pY + h.offsetHeight))		
	     return h; 
	  }
	}
	   ...
	   if (item)
	   {
	     ...
	     if (dragItem.colIdx != -1)
	     {
	       if (item.cellIndex == dragItem.colIdx + 1)
	         left = false;
	       else if (item.cellIndex == dragItem.colIdx - 1)
	         left = true;
	     }
	   }
	   else {
	          ...
	          if (item != -1)
 	          {
			 	    ...
			 	    if (dragItem.grpIdx != -1)	
				    {
				      if (item.cellIndex == dragItem.grpIdx + 1)
			                left = false;
			              else if (item.cellIndex == dragItem.grpIdx - 1)
			                left = true;
			            }
                                 }
                 }
...
function c1g_mouseDown(e)
{
	...
	if ((gridid) && c1g_getattr(document.getElementById(gridid), "AllowColMoving"))
	{
	  var hdr = c1g_mouseInHeader(e, gridid);
          if (hdr)
	     c1g_showDrag(hdr, gridid, e.pageX, e.pageY, true);	
	  else
	  {
	    hdr = c1g_mouseInPanel(e, gridid);
	    if ((hdr != -1) && (hdr))
               c1g_showDrag(hdr, gridid, e.pageX, e.pageY, false);
	  }
